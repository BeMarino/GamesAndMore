...
<?php
	...
	if(isLoggedIn()){
		...
		else echo '<script>alert("Non puoi accedere a quest\'area.");window.location="index.php";</script>';
				...
				<div class="row">
					<div class="col-md-12">
						...
						<li><a href="index.php">Home</a></li><?php if(isAdmin())echo'
							...
							<li class="active">ordini utente n: <?php echo $idUtente;?></li>
							</ul>
					</div>
				</div>
				<!-- /row -->
		...
		<div  style="margin-left:300px;width:500px">
			<!-- container -->
					...
					<?php if(isLoggedIn()){
							if($totale_ordini==0){?>
						<div class="row">
							<h3 align="center">Non ha ancora effettuato ordini</h3>
						</div>
							<?php }else{?>
						<!-- Order Details -->
						...
						<ul style="list-style-type:none;  margin-left:-150px;">
							...
							while($ordine=$ord->fetch_assoc()){
								...
								?>
							<li style="margin-top:30px;margin-left:-150px"><div class="order-details" style="width:600px;heigth: 250px;">	
								...
								while($row=$res->fetch_assoc()) {
									...
									?><div><?php echo $row['quantita'].'x'.$nome.'&nbsp'.$row['quantita']*$row['prezzo_unita'].'&#8364'; ?></div><?php
								}
								
								?>
								<br>
								<div><strong>TOTALE</strong></div>
								<div ><strong id="costoFinale" class="order-total"><?php echo $totale_prezzo.'&#8364  (Spese di spedizione incluse)';?></strong></div>
